...
export class ContextCreatorProvider implements vscode.TreeDataProvider<FileItem> {
    ...
    private loadPersistedState() {
        ...
        const state = this.context.globalState.get<Record<string, boolean>>('fileStates');
        if (state) {
        ...
        try {
            ...
            if (fileSizeKB > this.maxFileSizeWarningKB) {
                const proceed = await vscode.window.showWarningMessage(
                    `File "${path.basename(filePath)}" is ${Math.round(fileSizeKB)}KB, which exceeds the warning threshold (${this.maxFileSizeWarningKB}KB). This may impact performance.`,
                    'Select Anyway',
                    'Cancel'
                );
    ...
    private async toggleDirectoryChildren(dirPath: string, checked: boolean) {
        try {
            ...
            for (const entry of entries) {
                ...
                const filePath = path.join(dirPath, entry.name);
                const item = this.items.get(filePath) || new FileItem(
                    entry.name,
            ...
            vscode.window.showErrorMessage(`Error toggling directory: ${error instanceof Error ? error.message : error}`);
        }
    }
